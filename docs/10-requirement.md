...
_Updated for v2.19.1: Trunk NPC now draws two tiles lower with a wider shadow._
...
- **URS-021: Passing Trunk obstacle**
  ...
  - *Success*: A trunk glides from the left toward the right, displays 1.1× larger from its center with smoothing that preserves sprite detail, draws two tiles lower with a wider shadow, cannot be stood on, does not impede the player, kicks up dust like a slide, and appears above other characters when overlapping.
